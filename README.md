...
- `SCAN_PULL_REQUEST` – **Optional** Flag when scanner is used on `pull_request` events. 
Scans on Pull Requests are disabled by default as the action expects to be triggered by push events. 
